  ...
  return (
    <form className="space-y-6" onSubmit={(event) => { event.preventDefault(); void submit(event.currentTarget); }}>
      ...
      <section className="tg-card p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold">Presensi</h2><p className="text-sm tg-muted">Semua murid ditandai hadir secara default.</p></div><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold text-emerald-800">{students.length} murid</span></div>
