...
export function Topbar({ schoolName, multipleSchools }: { schoolName: string; multipleSchools: boolean }) {
  return (
    <header className="tg-surface sticky top-0 z-30 flex min-h-16 items-center justify-between border-b px-4 sm:px-6 lg:px-8">
      ...
      <div className="hidden min-w-0 lg:block">
        ...
        <div className="mt-1 flex items-center gap-2">
          <Building2 size={17} className="shrink-0 text-[var(--tg-primary)]" aria-hidden="true" />
          <p className="truncate text-sm font-bold">{schoolName}</p>
          {multipleSchools ? (
            <Link href="/school/select?change=1" className="text-xs font-bold text-[var(--tg-primary)]">
              Ganti
            </Link>
          ) : null}
        </div>
